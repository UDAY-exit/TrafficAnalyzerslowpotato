...
/**
 * AiRepository — Builds a short prompt from IPinfo data and calls
    ...
    suspend fun summarize(
        ...
        if (apiKey.isBlank() || apiKey == "YOUR_API_KEY_HERE") {
            return@withContext Result.failure(
                IllegalStateException("AI API key is not set in build.gradle.kts → AI_API_KEY.")
            )
        }
        ...
        runCatching {
            ...
            if (response.error != null) {
                val msg = response.error.message ?: "Unknown AI error"
                Log.e(TAG, "AI error: $msg")
                error(msg)
            }

            val text = response.firstText()
            Log.d(TAG, "Call #$callCount success — ${text?.length ?: 0} chars returned")
            text ?: error("AI returned an empty response.")
        }.onFailure { err ->
